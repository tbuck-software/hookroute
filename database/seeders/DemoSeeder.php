...
class DemoSeeder extends Seeder
{
    public function run(): void
    {
        ...
        $github = Source::factory()->for($project)->create(['name' => 'GitHub', 'slug' => 'github', 'last_received_at' => now()->subMinutes(4)]);
        $billing = Source::factory()->for($project)->create(['name' => 'Billing API', 'slug' => 'billing', 'last_received_at' => now()->subMinutes(18)]);
        $discord = Destination::factory()->for($project)->discord()->create(['name' => 'Operations Discord', 'last_delivered_at' => now()->subMinutes(4)]);
        $archive = Destination::factory()->for($project)->webhook()->create(['name' => 'Audit archive', 'last_delivered_at' => now()->subMinutes(8)]);
        ...
        foreach (range(1, 9) as $index) {
            ...
            $event = Event::factory()->for($project)->for($source)->create([
                ...
                'received_at' => now()->subMinutes($index * 7),
            ]);
            if ($source->is($github)) {
                foreach ([$githubDiscord, $githubArchive] as $connection) {
                    Delivery::factory()->for($event)->for($connection)->for($connection->destination)->create([
                        ...
                        'delivered_at' => $index === 4 && $connection->is($githubArchive) ? null : now()->subMinutes($index * 7 - 1),
                    ]);
                }
            }
        }
